...
def save_corners_to_csv(corners, output_path):
    """
    将角点 (x, y) 坐标保存为 CSV 表格（Excel 可直接打开）
    """
    with open(output_path, 'w') as f:
        f.write("Index,X,Y\n")  # 表头
        for idx, (x, y) in enumerate(corners):
            f.write(f"{idx},{x},{y}\n")
    print(f"角点坐标已保存到 {output_path}")


# 2. Harris 角点检测（不使用第三方库）
def computeHarrisCorners(px_array, image_width, image_height, k=0.04, do_nonmax_suppression=True):
    """
    ...
    Ixx = IPSmooth.computeGaussianAveraging(Ixx, image_width, image_height, kernel_size=5)
    Iyy = IPSmooth.computeGaussianAveraging(Iyy, image_width, image_height, kernel_size=5)
    Ixy = IPSmooth.computeGaussianAveraging(Ixy, image_width, image_height, kernel_size=5)
    ...
    # 5) 使用相对阈值进行角点初筛
    threshold = 1e7  # 按照题目要求，直接固定为 10的7次方
    corners = []
    for r in range(image_height):
        for c in range(image_width):
    ...
    print(f"Non-maximum suppression: {len(nms_corners)}")

    # 7. 排序提取 top 1000 strongest corners
    corner_responses = np.array(corner_responses)
    if len(nms_corners) > 1000:
        top_indices = np.argsort(corner_responses)[::-1][:1000]
        final_corners = [nms_corners[i] for i in top_indices]
    else:
        final_corners = nms_corners

    print(f"The top 1000 strongest corners: {len(final_corners)}")
...
def filter_corners(corners, image_width, image_height, width_border=10, height_border=40):
    # 剔除靠近图像边缘的角点
    filtered = []
    for (x, y) in corners:
        if width_border < x < (image_width - width_border) and height_border < y < (image_height - height_border):
            filtered.append((x, y))
    return filtered
...
def main():
    ...
    start = timer()
    px_array_left = IPSmooth.computeGaussianAveraging(px_array_left_original, image_width, image_height, kernel_size=5)
    px_array_right = IPSmooth.computeGaussianAveraging(px_array_right_original, image_width, image_height,
                                                       kernel_size=5)
    end = timer()
    ...
    harris_corners_left = computeHarrisCorners(px_array_left, image_width, image_height,
                                               k=0.04, do_nonmax_suppression=True)
    save_corners_to_csv(harris_corners_left, "left_image_corners.csv")

    harris_corners_right = computeHarrisCorners(px_array_right, image_width, image_height,
                                                k=0.04, do_nonmax_suppression=True)
    save_corners_to_csv(harris_corners_right, "right_image_corners.csv")
