...
# SUSAN角点检测算法
def computeSUSANCorners(px_array, image_width, image_height, radius=3, threshold=20, geometric_threshold=0.5, smooth=True, gaussian_kernel=3):
    """
    使用 SUSAN 角点检测算法替代 Harris 角点检测。

    参数:
    - px_array: 输入灰度图像的像素数组（二维列表）
    - image_width: 图像宽度
    - image_height: 图像高度
    - radius: SUSAN 探测圆的半径
    - threshold: 亮度相似度阈值（用于确定 USAN 区域）
    - geometric_threshold: 角点阈值，通常取 0.4~0.5
    ...
    if smooth:
        px_array = IPSmooth.computeGaussianAveraging(px_array, image_width, image_height, kernel_size=gaussian_kernel)

    # 创建一个存储角点响应值的数组
    response_map = np.zeros((image_height, image_width))

    # 定义 3x3 邻域的偏移（SUSAN 核心区域）
    offsets = [(-1, -1), (-1, 0), (-1, 1),
               (0, -1), (0, 0), (0, 1),
               (1, -1), (1, 0), (1, 1)]
    ...
    for y in range(1, image_height - 1):
        for x in range(1, image_width - 1):
            center_intensity = px_array[y][x]  # 中心像素值
            similar_count = 0  # 计算 USAN 区域大小

            # 遍历 3x3 邻域
            for dy, dx in offsets:
                ny, nx = y + dy, x + dx  # 邻域坐标
                neighbor_intensity = px_array[ny][nx]  # 邻域像素值

                # 计算亮度相似度
                if abs(center_intensity - neighbor_intensity) < threshold:
                    similar_count += 1  # 计数

            # 计算角点响应值（1 - USAN 区域 / 最大可能值）
            response = 1 - (similar_count / len(offsets))
            response_map[y, x] = response

    # 角点检测
    corners = []
    max_response = np.max(response_map)
    corner_threshold = geometric_threshold * max_response  # 角点筛选阈值

    for y in range(1, image_height - 1):
        for x in range(1, image_width - 1):
            if response_map[y, x] > corner_threshold:
                corners.append((x, y))  # 记录角点坐标 (x, y)
...
def ransacHomography(matches, num_iterations=500, inlier_threshold=5.0):
    """
    在原基础上增加:
    1) RANSAC 找到 best_H.
...
def main():
    ...
    # 1. 用 SUSAN 角点检测替代 Harris 角点检测
    SUSAN_corners_left = computeSUSANCorners(px_array_left, image_width, image_height)
    SUSAN_corners_right = computeSUSANCorners(px_array_right, image_width, image_height)

    # 过滤边缘角点，防止角点在图像边界
    SUSAN_corners_left = filter_corners(SUSAN_corners_left, image_width, image_height)
    SUSAN_corners_right = filter_corners(SUSAN_corners_right, image_width, image_height)
    SUSAN_corners_left=SUSAN_corners_left[:5000]
    SUSAN_corners_right = SUSAN_corners_right[:5000]

    print(f"Detected {len(SUSAN_corners_left)} corners in left image.")
    print(f"Detected {len(SUSAN_corners_right)} corners in right image.")

    # 2. 计算特征描述符
    window_size = 7
    descriptors_left = computeFeatureDescriptors(px_array_left, SUSAN_corners_left, image_width, image_height, window_size)
    descriptors_right = computeFeatureDescriptors(px_array_right, SUSAN_corners_right, image_width, image_height, window_size)
    ...
    # 3. 进行特征匹配
    matches = matchFeatures(descriptors_left, descriptors_right, SUSAN_corners_left, SUSAN_corners_right, distance_threshold=0.75)
    print(f"Found {len(matches)} raw matches.")

    # 4. 估计单应性变换矩阵
    best_H = ransacHomography(matches, num_iterations=1000, inlier_threshold=1.0)
    if best_H is None:
    ...
    stitched_inverse = warpAndStitchInverse(px_array_left, px_array_right, best_H, image_width, image_height)

    # 6. 角点可视化
    fig1, axs1 = pyplot.subplots(1, 2, figsize=(10, 5))
    axs1[0].imshow(px_array_left, cmap='gray')
    axs1[0].set_title("Left image with SUSAN corners")
    ...
    axs1[1].set_title("Right image with SUSAN corners")
